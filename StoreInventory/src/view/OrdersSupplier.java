...
public class OrdersSupplier extends Orders {
	...
	public OrdersSupplier() throws IOException {
		super();
		ordersList = ordersController_.showOrdersTable(1);
	}
	...
	public int showMenu() throws IOException {
		...
		while(validCommandFlag == 0) {
			...
			command = viewFunctions.validateInsertedData(1,3,command, "I want to: ", "! Invalid choice!. Please try again"); //check if user chose a valid option.
	...
	private boolean actionNavigate(int command) throws IOException {
		...
		case 1: //View pending orders table
			...
			//result = anotherActions();
			result = true;
			break;
		case 2: //Approved order
			System.out.println(viewFunctions.getSeperator());
			viewFunctions.showProgressBar(prevScreensTemp, "Approved order");
		...
		case 3: //Denied order 
			System.out.println(viewFunctions.getSeperator());
			viewFunctions.showProgressBar(prevScreensTemp, "Denied order");
			isEmptyOrders = isEmptyMap("pending");
			if(!isEmptyOrders) {
